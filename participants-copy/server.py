...
async def connect(sid, _):
    ...
    # Add player
    sio.logger.debug(f'[TODO] Add Player: {sid} to playerList')

    # Announce to everybody connected that a new player has connected
    sio.logger.debug(f'[TODO] Announce to everyone that player {sid} has connected (Pass playerList)')
...
async def disconnect(sid):
    ...
    sio.logger.debug(f'[TODO] Remove Player: {sid} from playerList')

    # announce to everybody connected that a player has disconnected
    # with payload of playerList
    sio.logger.debug(f'[TODO] Announce to everyone that player {sid} has disconnected (Pass playerList)')
...
async def button_pressed(sid):
    ...
    sio.logger.debug(f'[TODO] Generate randomTop and randomLeft (0 - 0.95)')

    # increase player score by 1
    sio.logger.debug(f'[TODO] Increase playerScore by 1')
    ...
    sio.logger.debug(f'[TODO] Announce to everyone that player {sid} has pressed the button (Pass randomTop, randomLeft, playerList)')
